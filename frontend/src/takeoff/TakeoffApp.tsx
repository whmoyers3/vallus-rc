...
type TakeoffBoundaryCandidate = {
  ...
  exteriorProfileArea: number;
  exteriorProfilePolygons?: TakeoffPoint[][];
...
function mirrorComponentHorizontal(component: TakeoffRoomComponent, axisWidth: number): TakeoffRoomComponent {
  return {
    ...
    wallProfilePolygons: component.wallProfilePolygons?.map((panel) => panel.map((point) => ({ ...point })).reverse()),
  };
}
...
function defaultComponent(surface: TakeoffRoomComponent["surface"], area: number): TakeoffRoomComponent {
  ...
  const openingDefaults = isOpeningSurface(surface)
    ? {
        width: 3,
        height: defaultOpeningHeight(surface),
        sillHeight: defaultOpeningSillHeight(surface),
        headHeight: defaultOpeningSillHeight(surface) + defaultOpeningHeight(surface),
      }
    : null;
  return {
    ...
    area: openingDefaults ? Number((openingDefaults.width * openingDefaults.height).toFixed(2)) : Math.max(0, Math.round(area)),
...
function isOpeningSurface(surface: TakeoffRoomComponent["surface"]): surface is "glass" | "door" {
  return surface === "glass" || surface === "door";
}

function isOpeningComponent(component: TakeoffRoomComponent) {
  return isOpeningSurface(component.surface);
}

function defaultOpeningSillHeight(surface: "glass" | "door") {
  return surface === "door" ? defaultDoorSillHeight : defaultWindowSillHeight;
}

function defaultOpeningHeight(surface: "glass" | "door") {
  return surface === "door" ? defaultDoorHeight : defaultWindowHeight;
}

function openingWidthForComponent(component: TakeoffRoomComponent) {
  const fallbackHeight = isOpeningSurface(component.surface) ? defaultOpeningHeight(component.surface) : defaultWindowHeight;
  if (component.width && component.width > 0) return component.width;
  if (component.area > 0 && fallbackHeight > 0) return Number(Math.max(1.5, component.area / fallbackHeight).toFixed(3));
  return 3;
}

function openingHeightForComponent(component: TakeoffRoomComponent) {
  if (component.height && component.height > 0) return component.height;
  const width = openingWidthForComponent(component);
  if (component.area > 0 && width > 0) return Number(Math.max(0.5, component.area / width).toFixed(3));
  return isOpeningSurface(component.surface) ? defaultOpeningHeight(component.surface) : defaultWindowHeight;
}

function openingVerticalSpan(component: TakeoffRoomComponent) {
  const surface = isOpeningSurface(component.surface) ? component.surface : "glass";
  const fallbackHeight = openingHeightForComponent(component);
  const fallbackSill = defaultOpeningSillHeight(surface);
  const hasSill = Number.isFinite(component.sillHeight);
  const hasHead = Number.isFinite(component.headHeight);
  const sillHeight = hasSill
    ? Math.max(0, component.sillHeight!)
    : hasHead
      ? Math.max(0, component.headHeight! - fallbackHeight)
      : fallbackSill;
  const headHeight = hasHead
    ? Math.max(sillHeight + 0.1, component.headHeight!)
    : sillHeight + fallbackHeight;
  return {
    sillHeight: Number(sillHeight.toFixed(3)),
    headHeight: Number(headHeight.toFixed(3)),
    height: Number(Math.max(0.1, headHeight - sillHeight).toFixed(3)),
  };
}
...
function componentIsWallGapFill(component: TakeoffRoomComponent) {
...
function componentDefinesWallProfileCoverage(component: TakeoffRoomComponent) {
  if (component.surface !== "wall") return false;
  if (!isCompassDirection(component.direction)) return false;
  if (!componentHasWallProfileGeometry(component)) return false;
  if (componentIsGeneratedCeilingWall(component)) return false;
  if (component.source === "open-to-above-envelope" || component.source === "connected-volume" || component.source === "band-joist") return false;
  return true;
}

function componentCountsTowardWallProfileLoad(component: TakeoffRoomComponent) {
  if (!componentDefinesWallProfileCoverage(component)) return false;
  if (component.loadExempt || component.adjacency === "conditioned") return false;
  return true;
}

function wallProfilePolygonsForComponent(component: TakeoffRoomComponent, segmentLengthValue: number, fallbackHeight: number) {
  const fallbackPolygon = [
    { x: component.spanStart ?? 0, y: component.zMin ?? 0 },
    { x: component.spanEnd ?? segmentLengthValue, y: component.zMin ?? 0 },
    { x: component.spanEnd ?? segmentLengthValue, y: component.zMax ?? fallbackHeight },
    { x: component.spanStart ?? 0, y: component.zMax ?? fallbackHeight },
  ];
  const polygons = component.wallProfilePolygons?.length ? component.wallProfilePolygons : [fallbackPolygon];
  return polygons
    .map((polygon) => dedupeProfilePoints(polygon).filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y)))
    .filter((polygon) => polygon.length >= 3 && polygonArea(polygon) > 0.05);
}

function wallProfileSpanRangeForComponent(component: TakeoffRoomComponent, segmentLengthValue: number, fallbackHeight: number) {
  const polygons = wallProfilePolygonsForComponent(component, segmentLengthValue, fallbackHeight);
  const xs = polygons.flatMap((polygon) => polygon.map((point) => point.x));
  const start = Number.isFinite(component.spanStart)
    ? component.spanStart!
    : xs.length > 0
      ? Math.min(...xs)
      : 0;
  const end = Number.isFinite(component.spanEnd)
    ? component.spanEnd!
    : xs.length > 0
      ? Math.max(...xs)
      : segmentLengthValue;
  return {
    start: clamp(Math.min(start, end), 0, segmentLengthValue),
    end: clamp(Math.max(start, end), 0, segmentLengthValue),
  };
}

function wallProfileComponentsForSegment(
  floor: TakeoffFloor,
  room: TakeoffRectRoom,
  segment: { a: TakeoffPoint; b: TakeoffPoint; direction: (typeof directionOptions)[number]; length: number },
  components = roomSurfaceComponents(room, "wall").filter(componentDefinesWallProfileCoverage),
) {
  const tolerance = Math.max(0.35, floor.scale.feetPerGrid * 0.35);
  return components.filter((component) => {
    if (component.direction !== segment.direction) return false;
    const profileSegment = wallSegmentForProfileComponent(floor, room, component);
    return Boolean(profileSegment && sharedSegmentLength(segment, profileSegment, tolerance) > 0.25);
  });
}

function wallProfileCoverageRangesForSegment(
  floor: TakeoffFloor,
  room: TakeoffRectRoom,
  segment: { a: TakeoffPoint; b: TakeoffPoint; direction: (typeof directionOptions)[number]; length: number },
  components = roomSurfaceComponents(room, "wall").filter(componentDefinesWallProfileCoverage),
  ...
  return wallProfileComponentsForSegment(floor, room, segment, components)
...
function uncoveredSegmentRanges(ranges: Array<{ start: number; end: number }>, segmentLengthValue: number) {
  const mergedRanges = mergeSegmentRanges(ranges, segmentLengthValue);
  const uncovered: Array<{ start: number; end: number }> = [];
  let cursor = 0;
  for (const range of mergedRanges) {
    if (range.start > cursor + 0.05) uncovered.push({ start: cursor, end: range.start });
    cursor = Math.max(cursor, range.end);
  }
  if (cursor < segmentLengthValue - 0.05) uncovered.push({ start: cursor, end: segmentLengthValue });
  return uncovered;
}

function wallProfileLoadCoverageAreaByDirection(floor: TakeoffFloor, room: TakeoffRectRoom) {
  const baseHeight = baseEnvelopeHeightForWallSuggestion(floor, room);
  const areas = new Map<(typeof directionOptions)[number], number>();
  const loadProfileComponents = roomSurfaceComponents(room, "wall").filter(componentCountsTowardWallProfileLoad);
  const segments = roomExteriorSegments(floor, room, { includeConditionedAdjacency: true });

  for (const segment of segments) {
    const componentPolygons = wallProfileComponentsForSegment(floor, room, segment, loadProfileComponents)
      .flatMap((component) => wallProfilePolygonsForComponent(component, segment.length, baseHeight));
    if (componentPolygons.length === 0) continue;
    const baseRectangle = pointsToClipPolygon([
      { x: 0, y: 0 },
      { x: segment.length, y: 0 },
      { x: segment.length, y: baseHeight },
      { x: 0, y: baseHeight },
    ]);
    const clippedPolygons = componentPolygons.flatMap((polygon) => intersection([pointsToClipPolygon(polygon)], [baseRectangle]));
    if (clippedPolygons.length === 0) continue;
    const [firstClippedPolygon, ...remainingClippedPolygons] = clippedPolygons;
    const covered = union(firstClippedPolygon, ...remainingClippedPolygons);
    const area = covered.reduce((sum, polygon) => sum + clipPolygonArea(polygon), 0);
    if (area > 0.05) areas.set(segment.direction, (areas.get(segment.direction) ?? 0) + area);
  }

  return new Map(Array.from(areas.entries()).map(([direction, area]) => [direction, Number(area.toFixed(3))]));
}

function baseWallAreaAfterProfileCoverage(floor: TakeoffFloor, room: TakeoffRectRoom, direction: (typeof directionOptions)[number], targetArea: number) {
  const profileArea = wallProfileLoadCoverageAreaByDirection(floor, room).get(direction) ?? 0;
  return Number(Math.max(0, targetArea - profileArea).toFixed(3));
}

function reconcileBaseWallAreasForProfileCoverage(floor: TakeoffFloor, room: TakeoffRectRoom) {
  const profileAreas = wallProfileLoadCoverageAreaByDirection(floor, room);
  const profileDirections = new Set(
    Array.from(profileAreas.entries())
      .filter(([, area]) => area > 0.5)
      .map(([direction]) => direction)
  );
  if (profileDirections.size === 0) return room;

  const suggestions = new Map(roomExteriorWallSuggestions(floor, room).map((entry) => [entry.direction, entry.area]));
  const adjacent = adjacentKindsByDirection(floor, room);
  const consumedBaseDirections = new Set<(typeof directionOptions)[number]>();
  const nextComponents: TakeoffRoomComponent[] = [];
  let changed = false;

  for (const component of roomComponents(room)) {
    if (!isCompassDirection(component.direction) || !profileDirections.has(component.direction) || !componentIsBaseWallArea(component)) {
      nextComponents.push(component);
      continue;
    }

    if (consumedBaseDirections.has(component.direction)) {
      changed = true;
      continue;
    }

    consumedBaseDirections.add(component.direction);
    const targetArea = suggestions.get(component.direction) ?? Math.max(0, component.area || 0);
    const baseArea = baseWallAreaAfterProfileCoverage(floor, room, component.direction, targetArea);
    if (baseArea <= 0.05) {
      changed = true;
      continue;
    }
    const recommendation = recommendedWallTreatment(adjacent.get(component.direction) ?? [], component.assembly || "W1");
    const nextComponent = {
      ...componentWithStableId(room.id, component),
      area: Math.round(baseArea),
      assembly: component.assembly || recommendation.assembly,
      adjacency: component.adjacency ?? recommendation.adjacency,
      boundary: component.boundary ?? boundaryForAdjacency(component.adjacency ?? recommendation.adjacency),
      source: component.source ?? "exterior-perimeter",
    };
    changed = changed || Math.abs((component.area || 0) - nextComponent.area) > 0.05;
    nextComponents.push(nextComponent);
  }

  for (const direction of profileDirections) {
    if (consumedBaseDirections.has(direction)) continue;
    const targetArea = suggestions.get(direction) ?? 0;
    const baseArea = baseWallAreaAfterProfileCoverage(floor, room, direction, targetArea);
    if (baseArea <= 0.5) continue;
    const recommendation = recommendedWallTreatment(adjacent.get(direction) ?? [], defaultWallAssemblyForAdjacency("outside"));
    nextComponents.push({
      ...defaultComponent("wall", Math.round(baseArea)),
      assembly: recommendation.assembly,
      direction,
      area: Math.round(baseArea),
      label: `${direction} ${recommendation.label.toLowerCase()}`,
      geometryLabel: `${Math.round(baseArea)} sf residual base wall after profile slices`,
      source: "exterior-perimeter",
      adjacency: recommendation.adjacency,
      boundary: boundaryForAdjacency(recommendation.adjacency),
    });
    changed = true;
  }

  return changed ? { ...room, components: nextComponents } : room;
}
...
function assignedWallAreaByDirection(floor: TakeoffFloor, room: TakeoffRectRoom) {
  ...
  for (const component of roomComponents(room)) {
    ...
    if (componentDefinesWallProfileCoverage(component)) continue;
    if (
  ...
  for (const [direction, area] of wallProfileLoadCoverageAreaByDirection(floor, room)) {
    walls.set(direction, (walls.get(direction) ?? 0) + area);
  }
  return walls;
}

function openingHostWallAreaByDirection(room: TakeoffRectRoom, floor?: TakeoffFloor, floors: TakeoffFloor[] = floor ? [floor] : []) {
  const walls = new Map<(typeof directionOptions)[number], number>();
  for (const component of roomComponents(room)) {
  ...
  if (floor) {
    for (const extension of openToAboveWallExtensionsForRoom(floor, room, floors)) {
      walls.set(extension.direction, (walls.get(extension.direction) ?? 0) + Math.max(0, extension.length * extension.addedHeight));
    }
  }
  return walls;
}
...
function roomWallReconciliation(floor: TakeoffFloor, room: TakeoffRectRoom, _floors: TakeoffFloor[] = [floor]) {
  ...
  const assignedWalls = assignedWallAreaByDirection(floor, room);
...
function wallAdjacentSpaceMismatches(floor: TakeoffFloor, room: TakeoffRectRoom) {
  const adjacent = adjacentKindsByDirection(floor, room);
  const profileDirections = new Set(
    roomSurfaceComponents(room, "wall")
      .filter(componentHasWallProfileGeometry)
      .map((component) => component.direction)
      .filter(isCompassDirection)
  );
  for (const candidate of boundaryCandidatesForFloor(floor).filter((candidate) => candidate.roomId === room.id)) {
    profileDirections.add(candidate.direction);
  }
  return roomSurfaceComponents(room, "wall").flatMap((component) => {
    ...
    if (profileDirections.has(component.direction)) return [];
...
function wallCanReceiveOpeningSubtractions(component: TakeoffRoomComponent) {
  if (component.surface !== "wall" || !isCompassDirection(component.direction)) return false;
  if (wallCanHostOpenings(component)) return true;
  if (component.source !== "open-to-above-envelope") return false;
  return component.adjacency == null || component.adjacency === "outside" || component.adjacency === "garage" || component.adjacency === "unknown";
}

function wallVerticalSpanForOpeningSubtraction(floor: TakeoffFloor, room: TakeoffRectRoom, component: TakeoffRoomComponent) {
  if (Number.isFinite(component.zMin) && Number.isFinite(component.zMax) && component.zMax! > component.zMin!) {
    return { zMin: Math.max(0, component.zMin!), zMax: Math.max(0, component.zMax!) };
  }
  return { zMin: 0, zMax: Math.max(0, baseEnvelopeHeightForWallSuggestion(floor, room)) };
}

function openingSubtractionsForRoom(room: TakeoffRectRoom) {
  return roomComponents(room)
    .filter((component) => isOpeningComponent(component) && isCompassDirection(component.direction))
    .map((component) => {
      const span = openingVerticalSpan(component);
      return {
        component,
        direction: component.direction as (typeof directionOptions)[number],
        area: Math.max(0, component.area || 0),
        zMin: span.sillHeight,
        zMax: span.headHeight,
        height: Math.max(0.1, span.height),
      };
    })
    .filter((entry) => entry.area > 0 && entry.zMax > entry.zMin);
}

function netWallComponentsOfOpenings(
  floor: TakeoffFloor,
  room: TakeoffRectRoom,
  wallComponents: TakeoffRoomComponent[],
) {
  const remainingOpenings = openingSubtractionsForRoom(room).map((entry) => ({ ...entry, remainingArea: entry.area }));
  return wallComponents.map((component) => {
    if (!wallCanReceiveOpeningSubtractions(component)) return exportedWallComponent(component);
    const grossArea = Math.max(0, component.area || 0);
    if (grossArea <= 0) return exportedWallComponent(component);
    const wallSpan = wallVerticalSpanForOpeningSubtraction(floor, room, component);
    if (wallSpan.zMax <= wallSpan.zMin) return exportedWallComponent(component);
    let subtract = 0;
    for (const opening of remainingOpenings) {
      if (opening.direction !== component.direction || opening.remainingArea <= 0) continue;
      const overlapHeight = intervalOverlap(wallSpan.zMin, wallSpan.zMax, opening.zMin, opening.zMax);
      if (overlapHeight <= 0) continue;
      const proportionalOpeningArea = opening.area * (overlapHeight / opening.height);
      const applied = Math.min(proportionalOpeningArea, opening.remainingArea, Math.max(0, grossArea - subtract));
      if (applied <= 0) continue;
      subtract += applied;
      opening.remainingArea = Math.max(0, opening.remainingArea - applied);
    }
...
function payloadComponentsForRoom(floor: TakeoffFloor, room: TakeoffRectRoom, floors: TakeoffFloor[]) {
  const expandedComponents = roomComponents(room)
    .filter((component) => !component.loadExempt)
    .flatMap((component) => payloadFloorComponentsForRoom(floor, floors, room, component));
  const wallIds = new Set(expandedComponents.filter((component) => component.surface === "wall").map((component) => component.id));
  const netWalls = netWallComponentsOfOpenings(floor, room, expandedComponents.filter((component) => component.surface === "wall"));
  const netWallsById = new Map(netWalls.map((component) => [component.id, component]));
  return expandedComponents
    .map((component) => wallIds.has(component.id) ? netWallsById.get(component.id) ?? component : exportedWallComponent(component))
    .filter((component): component is TakeoffRoomComponent => Boolean(component))
...
function payloadOpenToAboveEnvelopeComponentsForRoom(floor: TakeoffFloor, room: TakeoffRectRoom, floors: TakeoffFloor[]) {
  ...
  const extensionComponents = extensions
    ...
    .filter((component): component is TakeoffRoomComponent => component != null);
  return netWallComponentsOfOpenings(floor, room, extensionComponents)
    .filter((component) => Math.max(0, component.area || 0) > 0)
...
function pointAtSegmentDistance(segment: { a: TakeoffPoint; b: TakeoffPoint }, offset: number): TakeoffPoint {
  const length = Math.max(0.001, segmentLength(segment));
...
function distanceAlongSegment(segment: { a: TakeoffPoint; b: TakeoffPoint }, point: TakeoffPoint) {
  const dx = segment.b.x - segment.a.x;
  const dy = segment.b.y - segment.a.y;
...
function dedupeProfilePoints(points: TakeoffPoint[]) {
  return points.filter((point, index) => {
...
function adjacentWallProfileForSegment(
  space: TakeoffAdjacentSpace,
  room: TakeoffRectRoom,
  segment: { a: TakeoffPoint; b: TakeoffPoint },
  ...
  const topPoints = (
    isVaultCeilingType(adjacentCeilingInfo.ceilingType)
    ...
    const rawTop = isVaultCeilingType(adjacentCeilingInfo.ceilingType)
      ...
      : profileRange.zMax;
    return {
      x: Number(distanceAlongSegment(segment, point).toFixed(3)),
      y: Number(clamp(rawTop, zMin, room.ceilingHeight).toFixed(3)),
    };
  });
  const zMax = Math.max(...topPoints.map((point) => point.y), zMin);
  if (span.length <= 0.25 || zMax <= zMin + 0.25) return null;
  const profile = dedupeProfilePoints([
    { x: Number(span.start.toFixed(3)), y: Number(zMin.toFixed(3)) },
    { x: Number(span.end.toFixed(3)), y: Number(zMin.toFixed(3)) },
    ...topPoints.slice().reverse(),
  ]);
  const area = Number(polygonArea(profile).toFixed(3));
  ...
  return {
    ...
    profile,
    placement: {
      x: Number(((spanSegment.a.x + spanSegment.b.x) / 2).toFixed(3)),
      y: Number(((spanSegment.a.y + spanSegment.b.y) / 2).toFixed(3)),
    },
  };
}
  ...
  return roomExteriorSegments(floor, room, { includeConditionedAdjacency: true }).flatMap((segment) =>
    (floor.adjacentSpaces ?? []).flatMap((space) => {
      ...
      const complementProfilePolygons = wallProfileComplementPolygons(
        ...
        span.end,
        0,
        Math.max(baseHeight, wallProfile.zMax),
      );
  ...
  for (const room of floor.rooms) {
    ...
    for (const segment of roomExteriorSegments(floor, room)) {
      for (const space of floor.adjacentSpaces ?? []) {
        ...
        const wallProfile = adjacentWallProfileForSegment(space, room, segment, span, floor.defaultCeilingHeight ?? 9);
        if (!wallProfile) continue;
        const existingWallOverlapArea = wallProfile.area;
        const wholeSectionArea = Number((span.length * Math.max(baseHeight, wallProfile.zMax)).toFixed(3));
        const exteriorProfilePolygons = wallProfileComplementPolygons(
          wallProfile.profile,
          span.start,
          span.end,
          0,
        ...
        const exteriorProfileArea = Number(exteriorProfilePolygons.reduce((sum, polygon) => sum + polygonArea(polygon), 0).toFixed(3));
        const id = [
          "boundary",
          room.id,
          space.id,
        ...
        candidates.push({
          ...
          wallProfilePolygons: [wallProfile.profile],
          exteriorProfileArea,
          exteriorProfilePolygons,
          existingWallOverlapArea,
          wholeSectionArea,
          recommendedAdjacency: "attic",
          recommendedAssembly: "W3",
          recommendedBoundary: "attic_knee_wall",
          reason: `${space.name || adjacentSpaceLabel(space.kind)} closed ceiling/roof profile overlaps the conditioned wall from ${Number(wallProfile.zMin.toFixed(1))} ft to ${Number(wallProfile.zMax.toFixed(1))} ft over ${Number(span.length.toFixed(1))} lf.`,
        });
      }
    }
  }
...
function wallProfilePolygonBounds(polygons: TakeoffPoint[][]) {
  const points = polygons.flat();
  if (points.length === 0) return null;
  return polygonBounds(points);
}

function wallProfileRemainderGapFillSuggestionsForRoom(floor: TakeoffFloor, room: TakeoffRectRoom): WallGapFillSuggestion[] {
  const baseHeight = baseEnvelopeHeightForWallSuggestion(floor, room);
  ...
  for (const segment of roomExteriorSegments(floor, room, { includeConditionedAdjacency: true })) {
    ...
    const entries = segmentProfileComponents
      .map((component) => {
        const polygons = wallProfilePolygonsForComponent(component, segment.length, baseHeight);
        const range = wallProfileSpanRangeForComponent(component, segment.length, baseHeight);
        const maxY = Math.max(baseHeight, ...polygons.flatMap((polygon) => polygon.map((point) => point.y)));
        return { component, polygons, range, maxY };
      })
      .filter((entry) => entry.polygons.length > 0 && entry.range.end - entry.range.start > 0.05);
    if (entries.length === 0) continue;

    const mergedRanges = mergeSegmentRanges(entries.map((entry) => entry.range), segment.length);
    mergedRanges.forEach((range, rangeIndex) => {
      const overlappingEntries = entries.filter((entry) => entry.range.end > range.start + 0.05 && entry.range.start < range.end - 0.05);
      if (overlappingEntries.length === 0) return;
      const envelopeHeight = Math.max(baseHeight, ...overlappingEntries.map((entry) => entry.maxY));
      if (envelopeHeight <= 0.05) return;
      const envelope = [
        { x: range.start, y: 0 },
        { x: range.end, y: 0 },
        { x: range.end, y: envelopeHeight },
        { x: range.start, y: envelopeHeight },
      ];
      const envelopePolygon = pointsToClipPolygon(envelope);
      const coveredPolygons = overlappingEntries.flatMap((entry) =>
        entry.polygons.flatMap((polygon) => intersection([pointsToClipPolygon(polygon)], [envelopePolygon]))
      );
      if (coveredPolygons.length === 0) return;
      const [firstCoveredPolygon, ...remainingCoveredPolygons] = coveredPolygons;
      const covered = union(firstCoveredPolygon, ...remainingCoveredPolygons);
      const remainders = simplePolygonsFromMultiPolygon(difference([envelopePolygon], covered))
        .map(({ polygon }) => clipPolygonToPoints(polygon))
        .filter((polygon) => polygon.length >= 3 && polygonArea(polygon) > 0.5);
      if (remainders.length === 0) return;
      const area = Number(remainders.reduce((sum, polygon) => sum + polygonArea(polygon), 0).toFixed(3));
      if (area <= 0.5) return;
      const bounds = wallProfilePolygonBounds(remainders);
      const spanStart = Number((bounds?.x ?? range.start).toFixed(3));
      const spanEnd = Number(((bounds ? bounds.x + bounds.width : range.end)).toFixed(3));
      const zMin = Number((bounds?.y ?? 0).toFixed(3));
      const zMax = Number(((bounds ? bounds.y + bounds.depth : envelopeHeight)).toFixed(3));
      suggestions.push({
        key: `profile-remainder:${room.id}:${segment.direction}:${Math.round(range.start * 10)}:${Math.round(range.end * 10)}:${Math.round(envelopeHeight * 10)}:${rangeIndex}`,
        direction: segment.direction,
        area,
        label: `${segment.direction} exterior wall profile remainder`,
        geometryLabel: `Exterior wall remainder around generated profiles ${Number(range.start.toFixed(1))}-${Number(range.end.toFixed(1))} lf`,
        adjacency: "outside",
        ...
        zMax,
        placement: pointAtSegmentDistance(segment, (spanStart + spanEnd) / 2),
        wallProfilePolygons: remainders,
      });
    });
  }
...
function wallGapFillSuggestionsOverlap(first: WallGapFillSuggestion, second: WallGapFillSuggestion) {
  ...
  const zOverlap = intervalOverlap(first.zMin, first.zMax, second.zMin, second.zMax);
  if (spanOverlap <= 0.1 || zOverlap <= 0.1) return false;
  const smallerArea = Math.min(first.area, second.area);
  if (smallerArea <= 0.5) return false;
...
function wallGapFillSuggestionsForRoom(floor: TakeoffFloor, room: TakeoffRectRoom): WallGapFillSuggestion[] {
  const boundaryGapFills = boundaryCandidatesForFloor(floor)
    ...
    .map((candidate) => ({
      ...
      area: candidate.exteriorProfileArea,
      label: `${candidate.direction} exterior wall gap fill`,
      geometryLabel: `${candidate.adjacentSpaceName} exterior wall remainder ${candidate.zMin}-${candidate.zMax} ft`,
...
function buildValidation(
  ...
  for (const room of floor.rooms) {
    ...
    const missingGapFillSuggestions = wallGapFillSuggestionsForRoom(floor, room)
      .filter((suggestion) => !wallGapFillSuggestionApplied(room, suggestion));
    if (missingGapFillSuggestions.length > 0) {
      const missingArea = missingGapFillSuggestions.reduce((sum, suggestion) => sum + suggestion.area, 0);
      issues.push({
        severity: "warning",
        message: `${room.name || "Room"} has about ${Math.round(missingArea)} sf of building-envelope continuity gap not assigned. Review generated envelope sections to fill the geometry and choose the wall type.`,
        target: roomTarget,
      });
    }
    ...
    const openingHostWallAreas = openingHostWallAreaByDirection(room, floor, floors);
    ...
    for (const component of roomComponents(room)) {
      ...
      if (isOpeningComponent(component)) {
        const span = openingVerticalSpan(component);
        const modeledWallHeight = computedOpenToAboveHeight(floor, room, floors);
        if (span.headHeight > modeledWallHeight + 0.25) {
          issues.push({
            severity: "warning",
            message: `${room.name || "Room"} has ${componentSurfaceLabel(component.surface).toLowerCase()} ${component.label || component.assembly} head height ${Number(span.headHeight.toFixed(1))} ft above the modeled ${Number(modeledWallHeight.toFixed(1))} ft wall height.`,
            target: roomTarget,
          });
        }
      }
...
function openingMeshForComponent(component: TakeoffRoomComponent, room: TakeoffRectRoom, center: TakeoffPoint, material: THREE.Material, frameMaterial: THREE.Material) {
  ...
  const width = Math.max(1.5, openingWidthForComponent(component));
  const span = openingVerticalSpan(component);
  const wallHeight = Math.max(0.5, room.ceilingHeight);
  const explicitVerticalPlacement = Number.isFinite(component.sillHeight) || Number.isFinite(component.headHeight);
  const topPadding = component.surface === "glass" ? 0.75 : 0.25;
  const bottomPadding = 0.25;
  const fallbackHeight = component.surface === "door"
    ? Math.min(span.height, Math.max(0.5, wallHeight - topPadding))
    : Math.min(span.height, Math.max(0.5, wallHeight - topPadding - bottomPadding));
  const visibleHeight = explicitVerticalPlacement
    ? span.height
    : fallbackHeight;
  ...
  const openingBottom = explicitVerticalPlacement
    ? Math.max(0, span.sillHeight)
    : component.surface === "door"
      ? 0
      : clamp(defaultWindowSillHeight, bottomPadding, maxOpeningBottom);
...
function wallSegmentForProfileComponent(floor: TakeoffFloor, room: TakeoffRectRoom, component: TakeoffRoomComponent) {
  ...
  return segments
    .map((segment) => ({
      segment,
      distance: distance(closestPointOnSegment(component.placement!, segment.a, segment.b), component.placement!),
    }))
...
function wallGapFillMeshPartsForRoom(
  ...
  return roomSurfaceComponents(room, "wall").flatMap((component) => {
    ...
    const profiles = component.wallProfilePolygons?.length
      ...
      : [[
          { x: component.spanStart ?? 0, y: component.zMin ?? 0 },
          { x: component.spanEnd ?? segment.length, y: component.zMin ?? 0 },
          { x: component.spanEnd ?? segment.length, y: component.zMax ?? room.ceilingHeight },
          { x: component.spanStart ?? 0, y: component.zMax ?? room.ceilingHeight },
        ]];
    return profiles.flatMap((profile, index) => {
      const cleanProfile = dedupeProfilePoints(profile).filter((point) => Number.isFinite(point.x) && Number.isFinite(point.y));
      if (cleanProfile.length < 3) return [];
      const profileArea = Number(polygonArea(cleanProfile).toFixed(3));
      return [{
        mesh: createPanelMesh(
...
function TakeoffModelPreview({
  ...
  useEffect(() => {
    ...
    const wallMaterial = new THREE.MeshPhongMaterial({ color: 0x1f6fb2, depthWrite: false, transparent: true, opacity: modelComponentOpacity(0.42, 0.94), side: THREE.DoubleSide });
    ...
    const kneeWallMaterial = new THREE.MeshPhongMaterial({ color: 0xc9332c, depthWrite: false, transparent: true, opacity: modelComponentOpacity(0.36, 0.9), side: THREE.DoubleSide });
    ...
    const passiveWallMaterial = new THREE.MeshPhongMaterial({ color: 0x2f78b7, depthWrite: false, transparent: true, opacity: modelComponentOpacity(0.24, 0.58), side: THREE.DoubleSide });
    ...
    for (const otherFloor of floors) {
      ...
      for (const sourceRoom of otherFloor.rooms) {
        ...
        for (const component of roomSurfaceComponents(room, "glass").concat(roomSurfaceComponents(room, "door"))) {
          ...
          const openingRoom = { ...room, ceilingHeight: Math.max(baseWallHeight, room.ceilingHeight) };
    ...
    for (const [index, sourceRoom] of (activeOptions.visible ? floor.rooms : []).entries()) {
      ...
      for (const component of roomSurfaceComponents(room, "glass").concat(roomSurfaceComponents(room, "door"))) {
        if (component.surface === "glass" && !visibleLayers.windows) continue;
        if (component.surface === "door" && !visibleLayers.doors) continue;
        const openingRoom = { ...room, ceilingHeight: Math.max(baseEnvelopeHeightForWallSuggestion(floor, sourceRoom), room.ceilingHeight) };
  ...
  function updateSelectedOpeningComponent(patch: Partial<TakeoffRoomComponent>) {
    ...
    setSelectedSurface((current) => {
      if (!current || current.componentId !== selectedOpeningTarget.componentId) return current;
...
function validationSectionForIssue(issue: TakeoffValidationIssue): ValidationSection {
  ...
  if (message.includes("building-envelope continuity gap") || message.includes("generated envelope sections")) return "ceiling-geometry";
...
export function TakeoffApp() {
  ...
  function renderRoomLoadSketch(
    ...
    const wallHeightProject = (point: TakeoffPoint, height: number) => {
      const extraZ = Math.max(0, height - defaultCeilingHeight) * ceilingHeightScale;
      return screenProject(point, verticalRise / 2 + extraZ);
    };
    ...
    const generatedWallComponents = wallComponents.filter((component) =>
      (componentIsGeneratedCeilingWall(component) || componentRendersWallProfile(component)) &&
      !component.loadExempt &&
      component.adjacency !== "conditioned"
    );
    const wallProfileDirections = new Set(
      generatedWallComponents
        .filter(componentRendersWallProfile)
        .map((component) => component.direction)
        .filter(isCompassDirection)
    );
    ...
    const wallComponentForSketchEdge = (edge: { a: TakeoffPoint; b: TakeoffPoint }) => {
      ...
      if (wallProfileDirections.has(context.direction)) return null;
      return wallComponents.find((component) => {
        if (!componentIsBaseWallArea(component)) return false;
      ...
      <div key={`${mode}-${room.id}`} className={`takeoff-room-sketch takeoff-room-sketch--${mode}`}>
        ...
        <svg viewBox={`0 0 ${viewWidth} ${viewHeight}`} role="img" aria-label={`${room.name} load component sketch`}>
          ...
          {roomEdges.flatMap((edge) => {
            ...
            const wallTreatmentClass = wallComponent.adjacency === "garage"
              ? "takeoff-room-sketch-wall--garage"
              : componentIsAtticKneeWallTreatment(wallComponent)
                ? "takeoff-room-sketch-wall--attic"
                : "takeoff-room-sketch-wall--generated-exterior";
            return [(
              <g
                key={`${direction}-${edge.a.x}-${edge.a.y}`}
                className={`takeoff-room-sketch-wall takeoff-room-sketch-wall--load ${wallTreatmentClass} ${active ? "takeoff-room-sketch-panel--active" : ""}`}
          ...
          {roomEdges.flatMap((edge) => {
            ...
            return matchingGeneratedWalls.flatMap((component) => {
              const active = isActive("wall", direction);
              const generatedAdjacencyClass = componentIsAtticKneeWallTreatment(component)
                  ? "takeoff-room-sketch-wall--attic"
                  : componentIsExteriorWallTreatment(component)
                    ? "takeoff-room-sketch-wall--generated-exterior"
                    : "";
              if (componentRendersWallProfile(component)) {
                const gapSegment = wallSegmentForProfileComponent(floor, room, component);
                const tolerance = Math.max(0.35, floor.scale.feetPerGrid * 0.35);
                if (!gapSegment || sharedSegmentLength(edge, gapSegment, tolerance) <= 0.25) return null;
                const profiles = component.wallProfilePolygons?.length
                  ? component.wallProfilePolygons
                  : [[
                      { x: component.spanStart ?? 0, y: component.zMin ?? defaultCeilingHeight },
                      { x: component.spanEnd ?? gapSegment.length, y: component.zMin ?? defaultCeilingHeight },
                      { x: component.spanEnd ?? gapSegment.length, y: component.zMax ?? room.ceilingHeight },
                      { x: component.spanStart ?? 0, y: component.zMax ?? room.ceilingHeight },
                    ]];
                return profiles.map((profile, profileIndex) => {
                  const panel = dedupeProfilePoints(profile)
                    .map((profilePoint) => wallHeightProject(pointAtSegmentDistance(gapSegment, profilePoint.x), profilePoint.y));
                  if (panel.length < 3) return null;
                  const labelPoint = sketchLabelPoint(panel, (point) => point);
                  return (
                    <g
                      key={`generated-gap-${component.id}-${profileIndex}-${edge.a.x}-${edge.a.y}`}
    ...
    return (
      <div className="takeoff-component-list takeoff-component-list--workbench">
        {visibleComponents.map((component) => {
          ...
          const openingSpan = isOpeningComponent(component) ? openingVerticalSpan(component) : null;
          ...
          return (
            <div key={component.id} className={`takeoff-component-row takeoff-component-row--workbench ${isEditingComponent ? "takeoff-component-row--editing" : "takeoff-component-row--readonly"} ${componentRequiresDirection(component) ? "takeoff-component-row--directional" : ""} ${isStaleCeilingWall ? "takeoff-component-row--stale" : ""} ${isActiveComponentRow ? "takeoff-component-row--active" : ""}`}>
              ...
              <p className="takeoff-component-meta">
                ...
                {openingSpan ? ` · Sill ${openingSpan.sillHeight} ft · Head ${openingSpan.headHeight} ft` : ""}
  ...
  function approveRoomCeilingGeometry(
    ...
    setFloor((current) => ({
      ...
      rooms: current.rooms.map((room) => {
        ...
        const nextGapFillComponents = gapFillSuggestions.map((suggestion) => {
          ...
          return {
            ...
            label: suggestion.label,
            geometryLabel: suggestion.geometryLabel,
            source: "wall-gap-fill" as const,
            adjacency,
            boundary: loadExempt ? "partition" as const : boundaryForAdjacency(adjacency),
        ...
        const approvedRoom = isVaultCeilingType(ceilingInfo.ceilingType)
          ? {
              ...room,
  ...
  function applyWallComponentGeometrySuggestion(issue: TakeoffValidationIssue) {
    ...
    setFloor((current) => ({
      ...
      rooms: current.rooms.map((room) => {
        if (room.id !== roomId) return room;
        const direction = isCompassDirection(suggestion.direction) ? suggestion.direction : undefined;
        const targetArea = suggestion.area ?? 0;
        const baseArea = direction
          ? baseWallAreaAfterProfileCoverage(current, room, direction, targetArea)
          : targetArea;
        return {
          ...room,
          components: roomComponents(room)
            .map((component) => (
              roomComponentMatchesId(room.id, component, suggestion.componentId)
                ? {
                    ...componentWithStableId(room.id, component),
                    area: Math.round(Math.max(0, baseArea)),
                    assembly: suggestion.assembly ?? component.assembly,
                    adjacency: suggestion.adjacency ?? component.adjacency,
                    boundary: boundaryForAdjacency(suggestion.adjacency ?? component.adjacency ?? "outside"),
                    label: suggestion.label ?? component.label,
                    source: "exterior-perimeter" as const,
                  }
                : component
            ))
  ...
  function applySuggestedWallArea(
    ...
    setFloor((current) => ({
      ...
      rooms: current.rooms.map((room) => {
        ...
        const baseArea = baseWallAreaAfterProfileCoverage(current, room, suggestion.direction, suggestion.area);
        if (existing) {
          return {
            ...room,
            components: components
              .map((component) => roomComponentMatchesId(room.id, component, roomComponentStableId(room.id, existing))
                ? { ...componentWithStableId(room.id, component), assembly, adjacency, boundary: boundaryForAdjacency(adjacency), area: Math.round(baseArea), label: `${suggestion.direction} ${label.toLowerCase()}`, geometryLabel: suggestion.geometryLabel, source: "exterior-perimeter" as const }
                : component)
              .filter((component) => component.area > 0.05),
          };
        }
        if (baseArea <= 0.05) return room;
        return {
          ...room,
          components: [
            ...components,
    ...
    setFloors((currentFloors) => currentFloors.map((entry) => {
      ...
      return {
        ...
        rooms: entry.rooms.map((room) => {
          ...
          const nextComponents = components.map((component) => {
            if (matchesExteriorProfile(component)) {
              ...
              return {
                ...component,
                assembly: defaultWallAssemblyForAdjacency("outside"),
                area: Math.round(exteriorProfileArea),
                label: `${candidate.direction} exterior wall profile slice`,
                adjacency: "outside" as const,
                boundary: "exterior" as const,
                geometryLabel: `${candidate.adjacentSpaceName} exterior remainder`,
                placement: candidate.placement,
                spanStart: candidate.spanStart,
                spanEnd: candidate.spanEnd,
                zMin: 0,
                zMax: Math.max(candidate.zMax, component.zMax ?? candidate.zMax),
                wallProfilePolygons: candidate.exteriorProfilePolygons,
                loadExempt: false,
              };
            }
          ...
          const addedExteriorComponent: TakeoffRoomComponent | null = resolution === "slice" && exteriorProfileArea > 0.5 && (candidate.exteriorProfilePolygons?.length ?? 0) > 0 && !hasMatchingExteriorProfile
            ? {
                id: nextId("component-exterior-wall-profile"),
                surface: "wall",
                assembly: defaultWallAssemblyForAdjacency("outside"),
                direction: candidate.direction,
                area: Math.round(exteriorProfileArea),
                label: `${candidate.direction} exterior wall profile slice`,
                source: "wall-gap-fill",
                adjacency: "outside",
                boundary: "exterior",
                geometryLabel: `${candidate.adjacentSpaceName} exterior remainder`,
                spanStart: candidate.spanStart,
                spanEnd: candidate.spanEnd,
                zMin: 0,
                zMax: Math.max(candidate.zMax, room.ceilingHeight),
                placement: candidate.placement,
                wallProfilePolygons: candidate.exteriorProfilePolygons,
              }
            : null;
          ...
          const resolvedRoom = {
            ...
            components: [
              ...nextComponents,
              ...(addedExteriorComponent ? [addedExteriorComponent] : []),
              ...(addedBoundaryComponent ? [addedBoundaryComponent] : []),
            ],
          };
          return reconcileBaseWallAreasForProfileCoverage(entry, resolvedRoom);
        }),
      };
    }));
  ...
  function updateOpeningPlacement(patch: Partial<NonNullable<OpeningPlacement>>) {
    setOpeningPlacement((current) => {
      ...
      const nextSillHeight = Number(Math.max(0, patch.sillHeight ?? (surfaceChanged ? defaultOpeningSillHeight(nextSurface) : current.sillHeight ?? defaultOpeningSillHeight(nextSurface))).toFixed(3));
      let nextHeight = Number(Math.max(0.1, patch.height ?? (surfaceChanged ? defaultOpeningHeight(nextSurface) : current.height)).toFixed(3));
      let nextHeadHeight = Number(Math.max(nextSillHeight + 0.1, patch.headHeight ?? current.headHeight ?? nextSillHeight + nextHeight).toFixed(3));
      if (Object.prototype.hasOwnProperty.call(patch, "headHeight") && !Object.prototype.hasOwnProperty.call(patch, "height")) {
        nextHeight = Number(Math.max(0.1, nextHeadHeight - nextSillHeight).toFixed(3));
      } else if (
        surfaceChanged ||
        Object.prototype.hasOwnProperty.call(patch, "height") ||
        Object.prototype.hasOwnProperty.call(patch, "sillHeight")
      ) {
        nextHeadHeight = Number((nextSillHeight + nextHeight).toFixed(3));
      }
  ...
  function confirmOpeningPlacement() {
    ...
    const component: TakeoffRoomComponent = {
      ...
      sillHeight: openingPlacement.sillHeight,
      headHeight: openingPlacement.headHeight,
  ...
  function openOpeningEditor(room: TakeoffRectRoom, component: TakeoffRoomComponent) {
    ...
    setOpeningPlacement({
      ...
      width,
      height,
      sillHeight: span.sillHeight,
      headHeight: span.headHeight,
            ...
            <section className="takeoff-panel takeoff-room-profile-panel takeoff-room-profile-panel--workbench">
              ...
              {selectedRoom ? (
                (() => {
                  ...
                  const envelopeGapSuggestions = wallGapFillSuggestionsForRoom(floor, selectedRoom);
                  return (
                    <>
                        ...
                        {roomWorkbenchSections.ceiling && (
                          <div id={validationTargetId("ceiling-geometry")} className={`takeoff-workbench-section-body ${validationSectionClass("ceiling-geometry")}`}>
                            ...
                            <div className={`takeoff-ceiling-qa ${ceilingInfo.needsReview ? "takeoff-ceiling-qa--warning" : ""}`}>
                              ...
                              {(ceilingSuggestions.length > 0 || envelopeGapSuggestions.length > 0) && (
                                <div className="takeoff-ceiling-wall-suggestions">
                                  <div className="takeoff-component-head">
                                    <h4>Generated Envelope Sections</h4>
                                    <span>
                                      {ceilingSuggestions.filter((suggestion) => ceilingWallSuggestionApplied(selectedRoom, suggestion)).length + envelopeGapSuggestions.filter((suggestion) => wallGapFillSuggestionApplied(selectedRoom, suggestion)).length}
                                      {" / "}
                                      {ceilingSuggestions.length + envelopeGapSuggestions.length} added
                                    </span>
                                  </div>
                                  ...
                                  {envelopeGapSuggestions.map((suggestion) => {
                                    const key = `${selectedRoom.id}:${suggestion.key}`;
                                    const applied = wallGapFillSuggestionApplied(selectedRoom, suggestion);
                                    const selectedAdjacency = ceilingWallAdjacencies[key] || suggestion.adjacency;
                                    const suggestionNoLoad = selectedAdjacency === "conditioned";
                                    const selectedAssembly = suggestionNoLoad ? "NO_LOAD" : ceilingWallAssemblies[key] || suggestion.assembly || defaultWallAssemblyForAdjacency(selectedAdjacency);
                                    return (
                                      <button
                                        key={suggestion.key}
                                        className={`takeoff-ceiling-wall-suggestion ${applied ? "takeoff-ceiling-wall-suggestion--applied" : ""}`}
                                      ...
                                      >
                                        <span>
                                          ...
                                          <small>
                                            {Math.round(suggestion.area)} sf · {Number((suggestion.spanEnd - suggestion.spanStart).toFixed(1))} lf
                                            {` · ${suggestion.geometryLabel}`} · {suggestion.direction}-side · {selectedAdjacency}
                                          </small>
                                        </span>
                                        <select
                                          value={selectedAdjacency}
              ...
              <div className="takeoff-opening-grid">
                ...
                <label className="takeoff-opening-field">
                  ...
                  <input
                    ...
                    min="0"
                    step="0.1"
                    value={openingPlacement.sillHeight}
                    onChange={(event) => updateOpeningPlacement({ sillHeight: Number(event.target.value) })}
                  />
                </label>
                <label className="takeoff-opening-field">
                  Head height ft
                  <input
                    ...
                    min="0"
                    step="0.1"
                    value={openingPlacement.headHeight}
                    onChange={(event) => updateOpeningPlacement({ headHeight: Number(event.target.value) })}
                  />
                </label>
